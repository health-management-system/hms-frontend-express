...
describe('Patient Tests', () => {
    ...
    it('Test user can unsuccessfully update patient info', () => {
        // generate info
        const firstname = 'Dakota'
        const lastname = 'Wong'
        const dateOfBirth = '10/09/1995'
        ...
        cy.get('#healthCardNumber').clear().type(healthCardNumber)
        cy.get('#firstname').clear().type(firstname)
        cy.intercept('POST', 'http://localhost:4000/patient/register*', { statusCode: 400 }).as('update-info')
        cy.get('#submit-button').click()
            .wait('@update-info', {responseTimeout: 10000, requestTimeout:10000}).its('response.statusCode').should('eq', 400)
    })
